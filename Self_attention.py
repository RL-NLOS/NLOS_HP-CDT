...
class FinalPatchExpand(nn.Module):
    def __init__(self, dim, dim_scale=4):
        ...
        self.expand = nn.Linear(dim, (self.dim_scale ** 2 // 2) * dim, bias=False)
    ...
    def forward(self, x, H, W):
        ...
        return x, H * (self.dim_scale), W * (self.dim_scale)
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        if self.downsample is not None:
            ...
            assert H % 2 == 0 and W % 2 == 0, "图片尺寸必须被2^(len(depths)-1)整除"
            H, W = H // 2, W // 2
...
class BasicLayer_up(nn.Module):
    ...
    def __init__(self, dim, depth, num_heads, window_size, pe_dim, dim_scale,
                 mlp_ratio=4., qkv_bias=True, drop=0., attn_drop=0.,
                 drop_path=0., upsample=None):
        ...
        if upsample is not None:
            self.upsample = PatchExpand(dim=pe_dim, dim_scale=dim_scale)
        else:
            self.upsample = None
