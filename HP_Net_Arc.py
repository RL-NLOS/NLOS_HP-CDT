...
class Fusion_module(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        recal_input = recal_input + sp
        x1, x2 = torch.split(recal_input, c, dim=1)
        xo = self.channel_agg(torch.cat([x1, x2], dim=1))  ## 进行特征压缩 因为只计算一个特征的权重
        local_w = self.local_att(xo)  ## 局部注意力 即spatial attention
        global_w = self.global_att(xo)  ## 全局注意力 即channel attention
        return local_w * xo + global_w * xo
...
class Encoder(nn.Module):
    def __init__(self, patch_size=4, in_chans=3,
        ...
        # CNN branch
        self.inplanes_down = 32  # 第一个残差块的输入通道数
